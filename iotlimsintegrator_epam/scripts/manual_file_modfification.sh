...
psql << SQL
CREATE DATABASE iotlimsintegrator
    WITH
    OWNER = postgres
    ENCODING = 'UTF8'
    LC_COLLATE = 'C.UTF-8'
...
psql -c "\l" | grep iotlimsintegrator && echo "Database iotlimsintegrator created successfully" || echo "Failed to create database iotlimsintegrator"
EOF
fi
