...
function loadChart(data, graph_key, reload){
   ...
   if(!reload && data_str in chart_scripts){
      ...
      url = "{% url 'dashboard:custom-chart-data' %}" + graph_key + "/";
      if(reload)
...
function loadAnalyticsChart(chart_key){
   ...
   $('.admin_charts').hide();
   var url = "{% url 'dashboard:custom-chart-analytics-without-key' %}?graph_key=" + chart_key;
   $("#chart_element_" + chart_key + ".notloaded").load(url, function(){
